...
CREATE TABLE IF NOT EXISTS `Flippelis`.`USERACCOUNT` (
  ...
  UNIQUE INDEX `UK_USRACCOUNT_UsrEmail` (`Email` ASC) VISIBLE);
...
CREATE TABLE IF NOT EXISTS `Flippelis`.`PRODUCT` (
  ...
  `Characters` VARCHAR(200) CHARACTER SET 'utf8mb4' NULL,
...
CREATE TABLE IF NOT EXISTS `Flippelis`.`SERIES` (
  `ProdID` INT NOT NULL,
  `TotalEpisodes` TINYINT UNSIGNED NOT NULL,
...
INSERT INTO PRODUCT (ProdID, Title, Rate, ReleaseDate, Characters, Synopsis, Genre, Prizes, Collect, Trailer, Image, FDub, Platf, Compositor, BSO, Director, Producer)
...
INSERT INTO PRODUCT (ProdID, Title, Rate, ReleaseDate, Characters, Synopsis, Genre, Prizes, Collect, Trailer, Image, FDub, Platf, Compositor, BSO, Director, Producer)
VALUES (523456789, 'Como conoci a vuestra madre', '2005-09-19', 'https://www.sensacine.com/series/serie-446/temporada-1067/reparto/', 'Ted se ha enamorado. Todo comenzó cuando su mejor amigo, Marshall confiesa que planea proponerle matrimonio a su novia de muchos años Lily, una profesora del jardín infantil. De repente, Ted se da cuenta que tiene tomar acción para encontrar el amor verdadero. Ayudándole en su búsqueda está Barney, un amigo incondicional y frecuentemente alocado en sus opiniones, en sus elecciones para el vestuario y su forma infalible de conocer mujeres. Cuando Ted conoce a Robin, está seguro de haberse enamorado a primera vista pero su romance se transforma en una amistad, la voz de Bob Saget ("Full House") relata la historia a través de recuerdos.', 'Comedia', '12', 100000000, 'https://www.youtube.com/watch?v=aJtVL2_fA5w', 'https://pics.filmaffinity.com/how_i_met_your_mother_tv_series-740256945-mmed.jpg' ,'https://www.eldoblaje.com/datos/FichaPelicula.asp?id=10807', 'Disney Plus', 'Compositor John Swihart', 'Our whole universe... A no esta no era', 'Carter Bays', 'CBS Productions');
INSERT INTO SERIES (ProdID, TotalSeasons, TotalEpisodes, EpDuration)
